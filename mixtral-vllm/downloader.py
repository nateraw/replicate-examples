...
class Downloader:
    def __init__(self, concurrency: t.Optional[int] = None) -> None:
        if not concurrency:
    ...
    async def get_remote_file_size(self, url) -> "tuple[URL, int]":
        # try:
    ...
    async def download_chunk(
        self, url, start: int, end: int, buffer_view: memoryview
    ) -> None:
        async with self.sem:
            for i in range(5):
                headers = {"Retry-Count": str(i)} if i else {}
    ...
    async def download_file(self, url) -> mmap.mmap:
        self.retries = 0
    ...
    async def maybe_download_files_to_disk(
        self, path: str, remote_path: str, filenames: t.List[str]
    ) -> None:
        ...
        if not os.path.exists(path):
            ...
            missing_files = [file for file in filenames if file not in local_files]
